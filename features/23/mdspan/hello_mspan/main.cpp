...
int main(int argc, char **argv) {
  ...
  std::cout << "use multi dimensional access operator[] (supported in C++23)."
            << std::endl;
#else
  ...
  if (are_equal) {
    std::cout << "mdspan3D maps to the same linear index like mdspan1D"
              << std::endl;
  } else {
    std::cout << "mdspan3D maps to a different linear index like mdspan1D"
              << std::endl;
  }
